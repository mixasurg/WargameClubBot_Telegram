...
public class BookingService {
    ...
    public Booking create(BookingCreateRequest request) {
        ...
        TableAllocationService.AllocationSnapshot allocationSnapshot = tableAllocationService.allocateForRange(
                request.startAt(),
                request.endAt(),
                request.tableUnits(),
                request.tableId()
        ...
        if (army != null && army.isClubShared()
                && hasArmyConflict(allocationSnapshot.overlappingBookings(), army.getId())) {
            throw new ConflictException("Армия уже забронирована на это время");
        }

        List<TableAllocationService.TableAllocation> allocations = allocationSnapshot.allocations();
        String allocationsJson = tableAllocationService.serializeAllocations(allocations);
    ...
    public List<Booking> findOverlapping(OffsetDateTime from, OffsetDateTime to, Long tableId) {
        ...
        return bookings.stream()
                .filter(booking -> tableAllocationService.bookingHasTable(booking, tableId))
                .toList();
    }
