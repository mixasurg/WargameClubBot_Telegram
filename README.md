...
- Поддерживает открытые брони (`OPEN`) в режиме "ищу соперника", присоединение второго игрока (`join`) и автоснятие просроченных заявок.
...
- Поддерживает сценарий открытых игр: просмотр списка (`/open`), присоединение кнопкой и ручной командой (`/join <bookingId> [faction]`).
...
- `/open` - показать список открытых игр на 14 дней
- `/join <bookingId>` - присоединиться к открытой игре
- `/join <bookingId> <фракция>` - присоединиться и сразу указать фракцию
...
- `GET /api/bookings/open?from=...&to=...&game=...`
...
- Для открытой брони join доступен до `joinDeadlineAt` (по умолчанию `startAt - 12h`).
- Просроченные открытые брони автоматически снимаются планировщиком.
